...
- Sensitive logging: prompts and model outputs are written to `lab/apps/vuln_rag_agent/logs/`.
...
```bash
ENABLE_TOOL_GATE=1 ENABLE_RETRIEVAL_FIREWALL=1 python3 lab/apps/vuln_rag_agent/server.py --port 8000
```

These defenses are implemented in `lab/defenses/` and are meant to be measurable (re-run the Day001 attack pack).
