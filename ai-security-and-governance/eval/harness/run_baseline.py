...
def _start_server(port: int, mode: str) -> subprocess.Popen[bytes]:
    ...
    return subprocess.Popen(
        [sys.executable, str(ROOT / "apps/vuln_rag_agent/server.py"), "--port", str(port)],
        cwd=str(ROOT),
        env=env,
...
def main() -> int:
    ...
    parser.add_argument("--attacks-dir", default=str(ROOT / "attacks"))
    ...
    if not cases:
        raise SystemExit("No Day001 cases found (expected attacks/**/day001_cases.jsonl).")
