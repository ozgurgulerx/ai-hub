...
```bash
cat eval/baseline_metrics.vuln.json | python3 -m json.tool | less
cat eval/baseline_metrics.defended.json | python3 -m json.tool | less
```
...
v = json.loads(Path("eval/baseline_metrics.vuln.json").read_text())
d = json.loads(Path("eval/baseline_metrics.defended.json").read_text())
